// Dev-only `window.__driveline*` hook surface — extracted from `App.tsx`
// (SHELL-01). Playwright drives all smoke tests and the T2.4 `openFiles` drop
// test through this. The real `onDrop` handler in `App.tsx` calls exactly the
// same store actions.
//
// The hook NAMES and SHAPES are a stable contract: the e2e specs in
// `apps/e2e/` depend on them. Renaming or reshaping one is a breaking change.
// This module only relocates the registration — names, shapes, gating
// (`import.meta.env.DEV`) and behaviour are identical to the previous inline
// definitions.
...
import { isCursorGated } from "./timeline/playback";
...
export interface DevHookDeps {
  /** The data-core worker proxy. */
  dc: Remote<DataCoreApi>;
  /** The video-decode worker proxy. */
  vd: Remote<VideoDecodeApi>;
  /** Live handle to the FlexLayout workspace (panel-add / layout ops). */
  workspaceRef: { current: WorkspaceHandle | null };
}

/**
 * Register the dev-only `window.__driveline*` hook surface and return a
 * cleanup that removes it. Behaviour is identical to the previous inline
 * `App.tsx` registration — this is a pure relocation. The caller is
 * responsible for the `import.meta.env.DEV` gate (mirroring the previous
 * inline gate), so production builds tree-shake this module out entirely.
 */
    ...
    installFailingEngine: async (kind, bestRecipeJson) => {
      ...
      llm.setFormatAgentEngineFactory(() => ({
        async run({ validateLocally, onProgress, signal }) {
          ...
          const k =
            kind === "acceptance-gate" ? "acceptance-gate" : "iteration-cap";
