...
// added.channels preserves the order you sent, so index by position. (The
// returned .name is the DISPLAY name and comes back leading-slashed, e.g.
// "/gps/latitude" — don't try to match it against the un-slashed input name.)
const [latId, lonId] = added.channels.map((c) => c.id);
const panelId = agent.createPanel("map");
agent.setMapBinding(panelId, latId, lonId);
\`\`\`
...
### 3D scene geometry — point clouds, boxes, trajectories, road maps

The \`scene\` panel renders 3D geometry channels. A channel's \`kind\` (from
...
Map geometry is STATIC (one frame at ts=0): bind it to a scene panel once and
it renders without scrubbing the cursor.
...
- \`getSessionSnapshot()\` →
  \`{ cursorNs, playing, speed, globalRange: { startNs, endNs } | null }\` (ns as
  strings; \`globalRange\` covers every loaded source, including yours — use
...
You don't have to drive the transport (or open any panel) to inspect a frame.
These decode/sample at an explicit timestamp, off the live playback path, so a
human can keep watching uninterrupted while you analyse a different instant:

- \`captureVideoFrameAt(channelId, ns)\` → \`{ channelId, cameraName, ptsNs,
  width, height, dataUrl }\` — the camera frame nearest (at/before) \`ns\`,
  decoded on a throwaway decoder and returned as a PNG \`data:\` URL. Resolves
  \`null\` for an unknown/non-video channel or a \`ns\` with no covering frame.
  (\`captureVideoFrame(panelId?)\` is now an async alias that decodes a live
  video panel's camera frame at the CURRENT cursor via this same off-thread
  path — the panel's canvas is owned by the decode worker and can't be read
  back. It resolves \`{ channelId, cameraName, ptsNs, width, height, dataUrl }\`
  or \`null\`. Prefer \`...At\` when you want a specific timestamp.)
- \`snapshotAt(ns)\` → \`{ tsNs, cameras[], pointClouds[], scalars[], channels[] }\`
  — one call for everything at \`ns\`: a decoded frame per camera, the active
  LiDAR \`spinTsNs\` per point-cloud channel (fetch its points with
  \`fetchChannelRange(channelId, spinTsNs, spinTsNs+1)\`), every scalar's value
  at \`ns\`, and the channel inventory. Always resolves a bundle (empty arrays
