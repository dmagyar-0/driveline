...
export const AGENT_SKILL = `# Driveline — Bring Your Own Agent (BYOA) skill (v5)
...
- \`captureVideoFrameAt(channelId, ns)\` → \`{ channelId, cameraName, ptsNs,
  ...
  (\`captureVideoFrame(panelId?)\` still exists but only reads whatever is
  currently on a panel's canvas — prefer \`...At\` for frame-accurate, headless
  reads.)
