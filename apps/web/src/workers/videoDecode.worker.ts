...
// Frame-pacing / smoothness summary, re-exported from its own module
// (`videoCadence.ts`) so existing consumers — `App.tsx`'s `videoCadence` dev
// hook and `VideoPanel.tsx` — keep importing `CadenceSummary` from this worker
// module unchanged after the WAL-01 extraction.
...
// Frame-pacing / smoothness instrumentation (WAL-01: extracted to
// `videoCadence.ts`). The decode state machine drives this tracker from the
// blit seam (`recordPaint` in `blitForCursor`), the 200 Hz blit clock
// (`recordTick`), the scrub/seek snap (`noteScrubReanchor`), and every temporal
// discontinuity (`reset` in `flushBlitQueue` / play-start). It reads the SAME
// worker clock (`workerNow`) and is handed `playbackSpeed` at compute time, so
// the metrics, the `PACING {json}` shape, and timing are byte-identical to the
// previous inline version — this is a pure move off the hot path's owner.
const cadence = createCadenceTracker(workerNow);
...
function postBlitStatus(firstBlit: boolean, seekBlit: boolean): void {
  ...
  const status: BlitStatus = {
    ...
    firstBlit,
    seekBlit,
    cadence: cadence.snapshot(playbackSpeed),
  };
...
function blitForCursor(): void {
  ...
  cadence.recordPaint(target.ptsNs, w, blitCursorClamped);
...
export const videoDecodeApi = {
  ...
  getCadence(): CadenceSummary {
    return cadence.compute(playbackSpeed);
  },
  getCadenceTrace(): CadenceTrace {
    return cadence.trace();
  },
  resetCadence(): void {
    cadence.reset();
  },
